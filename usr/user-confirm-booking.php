...
if (isset($_POST['book_vehicle'])) {
    ...
    if ($result->num_rows > 0) {
        ...
        $query = "UPDATE tms_user SET u_car_type=?, u_car_bookdate=?, u_car_regno=?, u_car_book_status=?, pickup_location=?, drop_location=? WHERE u_id=?";
        $stmt = $mysqli->prepare($query);
        $stmt->bind_param('ssssssi', $u_car_type, $u_car_bookdate, $u_car_regno, $u_car_book_status, $pickup_location, $drop_location, $u_id);
        $stmt->execute();
        if ($stmt) {
